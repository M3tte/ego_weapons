...
public class SolemnLamentBurstHit extends MetaParticle {
    ...
    public SolemnLamentBurstHit(ClientWorld level, double x, double y, double z, double width, double height, double _null) {
        ...
        for (int i = 0; i < 2; i++) {
            this.level.addParticle(TCorpParticleRegistry.SOLEMN_LAMENT_FIRE_DEPARTED.get(), this.x + randBetween(-1f, 1f), this.y + 1 + randBetween(-0.5f, 0.5f), this.z + randBetween(-0.5f, 0.5f), 0.0D, 0.0D, 0.0D);
            this.level.addParticle(TCorpParticleRegistry.SOLEMN_LAMENT_FIRE_LIVING.get(), this.x + randBetween(-1f, 1f), this.y + 1 + randBetween(-0.5f, 0.5f), this.z + randBetween(-0.5f, 0.5f), 0.0D, 0.0D, 0.0D);
        }
        double d = 0.2F;
        ...
        for(int i = 0; i < 15; i++) {
            ...
            this.level.addParticle(TCorpParticleRegistry.SOLEMN_LAMENT_DEPARTED_BUTTERFLY.get(), this.x, this.y + 1, this.z, particleMotionX * 0.1, particleMotionY * 0.25, particleMotionZ * 0.1);
            this.level.addParticle(TCorpParticleRegistry.SOLEMN_LAMENT_DEPARTED_BUTTERFLY.get(), this.x, this.y + 1, this.z, particleMotionX * 0.05, particleMotionY * 0.25, particleMotionZ * 0.05);
            this.level.addParticle(TCorpParticleRegistry.SOLEMN_LAMENT_LIVING_BUTTERFLY.get(), this.x, this.y + 1, this.z, particleMotionX * 0.05, particleMotionY * 0.25, particleMotionZ * 0.05);
        }
    }
