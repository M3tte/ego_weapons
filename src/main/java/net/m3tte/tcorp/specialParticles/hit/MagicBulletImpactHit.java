...
@OnlyIn(Dist.CLIENT)
public class MagicBulletImpactHit extends MetaParticle {
    public MagicBulletImpactHit(ClientWorld level, double x, double y, double z, double width, double height, double _null) {
        super(level, x, y, z);
        ...
        this.level.addParticle(TCorpParticleRegistry.MAGIC_BULLET_IMPACT.get(), this.x, this.y + 1f, this.z, 0.0D, 0.0D, 0.0D);
        double d = 0.2F;
    ...
    public static class Provider implements IParticleFactory<BasicParticleType> {
        ...
        public Particle createParticle(BasicParticleType typeIn, ClientWorld levelIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            MagicBulletImpactHit particle = new MagicBulletImpactHit(levelIn, x, y, z, xSpeed, ySpeed, zSpeed);
            return particle;
        }
    }
}