...
@OnlyIn(Dist.CLIENT)
public class MimicryDashHit extends MetaParticle {
    public MimicryDashHit(ClientWorld level, double x, double y, double z, double width, double height, double _null) {
        super(level, x, y, z);
        this.x = x + (this.level.random.nextDouble() - 0.5D) * width;
        this.y = y + (height) * 0.5;
        this.z = z + (this.level.random.nextDouble() - 0.5D) * width;
        this.level.addParticle(TCorpParticleRegistry.MIMICRY_DASH_STRIKE.get(), this.x, this.y + 1, this.z, 0.0D, 0.0D, 0.0D);
        ...
        for(int i = 0; i < 8; i++) {
            ...
            d = d * (this.level.random.nextBoolean() ? 1.0D : -0.3D);
            this.level.addParticle(EpicFightParticles.BLOOD.get(), this.x, this.y + 1, this.z, particleMotionX, particleMotionY, particleMotionZ);
        }
    }
    ...
    public static class Provider implements IParticleFactory<BasicParticleType> {
        ...
        public Particle createParticle(BasicParticleType typeIn, ClientWorld levelIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            MimicryDashHit particle = new MimicryDashHit(levelIn, x, y, z, xSpeed, ySpeed, zSpeed);
            return particle;
        }
    }
}