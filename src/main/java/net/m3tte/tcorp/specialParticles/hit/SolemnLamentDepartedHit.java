...
@OnlyIn(Dist.CLIENT)
public class SolemnLamentDepartedHit extends MetaParticle {
    public SolemnLamentDepartedHit(ClientWorld level, double x, double y, double z, double width, double height, double _null) {
        super(level, x, y, z);
        this.x = x + (this.level.random.nextDouble() - 0.5D) * width;
        this.y = y + (height) * 0.5;
        this.z = z + (this.level.random.nextDouble() - 0.5D) * width;
        this.level.addParticle(TCorpParticleRegistry.SOLEMN_LAMENT_FIRE_DEPARTED.get(), this.x, this.y + 1, this.z, 0.0D, 0.0D, 0.0D);
        double d = 0.2F;
        ...
        for(int i = 0; i < 8; i++) {
            ...
            this.level.addParticle(TCorpParticleRegistry.SOLEMN_LAMENT_DEPARTED_BUTTERFLY.get(), this.x, this.y + 1, this.z, particleMotionX * 0.1, particleMotionY * 0.1, particleMotionZ * 0.1);
        }
    }
    ...
    public static class Provider implements IParticleFactory<BasicParticleType> {
        ...
        public Particle createParticle(BasicParticleType typeIn, ClientWorld levelIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            SolemnLamentDepartedHit particle = new SolemnLamentDepartedHit(levelIn, x, y, z, xSpeed, ySpeed, zSpeed);
            return particle;
        }
    }
}