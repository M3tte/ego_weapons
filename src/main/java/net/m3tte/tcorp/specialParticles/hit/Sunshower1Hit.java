...
@OnlyIn(Dist.CLIENT)
public class Sunshower1Hit extends MetaParticle {
    public Sunshower1Hit(ClientWorld level, double x, double y, double z, double width, double height, double _null) {
        super(level, x, y, z);
        ...
        this.level.addParticle(TCorpParticleRegistry.SUNSHOWER_AUTO1_STRIKE.get(), this.x, this.y + 1, this.z, 0.0D, 0.0D, 0.0D);
        double d = 0.2F;
    ...
    public static class Provider implements IParticleFactory<BasicParticleType> {
        ...
        public Particle createParticle(BasicParticleType typeIn, ClientWorld levelIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            Sunshower1Hit particle = new Sunshower1Hit(levelIn, x, y, z, xSpeed, ySpeed, zSpeed);
            return particle;
        }
    }
}