...
@OnlyIn(Dist.CLIENT)
public class AtelierShotgunHitEffect extends MetaParticle {
    public AtelierShotgunHitEffect(ClientWorld level, double x, double y, double z, double width, double height, double _null) {
        super(level, x, y, z);
        ...
        this.level.addParticle(TCorpParticleRegistry.ATELIER_SHOTGUN_IMPACT.get(), this.x, this.y + 1, this.z, 0.0D, 0.0D, 0.0D);
        double d = 0.2F;
    ...
    public static class Provider implements IParticleFactory<BasicParticleType> {
        ...
        public Particle createParticle(BasicParticleType typeIn, ClientWorld levelIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            AtelierShotgunHitEffect particle = new AtelierShotgunHitEffect(levelIn, x, y, z, xSpeed, ySpeed, zSpeed);
            return particle;
        }
    }
}