...
import static net.m3tte.ego_weapons.client.renderLayers.PatchedAccessoryRenderLayer.getArmorVertexBuilder;
...
public abstract class WearableRenderer<R extends LivingEntity, S extends BipedModel<R>, W extends BipedModel<R>> {
    ...
    public void render(R living, ItemStack wearable, S parentModel, MatrixStack matrixStack, IRenderTypeBuffer buffer, int combinedLight, float animationPosition, float animationSpeed, float partialTick) {
        ...
        if (model != null && texture != null) {
            ...
            // IVertexBuilder ivertexbuilder = ItemRenderer.getArmorFoilBuffer(buffer, EpicFightRenderTypes.animatedArmor(texture, false), true, true);
            IVertexBuilder vertexConsumer = getArmorVertexBuilder(buffer, RenderType.armorCutoutNoCull(texture));

            model.renderToBuffer(matrixStack, vertexConsumer, combinedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
            matrixStack.popPose();
        }
    }
