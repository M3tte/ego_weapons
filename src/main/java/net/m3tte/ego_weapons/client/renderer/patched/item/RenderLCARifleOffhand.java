...
@OnlyIn(Dist.CLIENT)
public class RenderLCARifleOffhand extends RenderItemBase {

    public RenderLCARifleOffhand() {
    }
    ...
    public void renderItemInHand(ItemStack stack, LivingEntityPatch<?> entitypatch, Hand hand, IRenderTypeBuffer buffer, MatrixStack poseStack, int packedLight) {
        ...
        if (hand.equals(Hand.MAIN_HAND)) {
            ...
            if (stack.getOrCreateTag().getInt("forceHandhold") > 0) {
                modelMatrix = new OpenMatrix4f(this.offhandCorrectionMatrix);
                ...
                poseStack.pushPose();
                this.mulPoseStack(poseStack, modelMatrix);
                poseStack.translate(-0.1f,-0.33f,0.25);
                poseStack.mulPose(Vector3f.XP.rotationDegrees(0.0F));
                poseStack.mulPose(Vector3f.YP.rotationDegrees(155.0F));
