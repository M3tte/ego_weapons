...
public class EntityTick {
	...
	public static void executeProcedure(Map<String, Object> dependencies) {
		...
		if (entity.tickCount % 101 == 0) {
			if (entity.getPersistentData().contains("dialogueMetaData")) {
				CompoundNBT data  = entity.getPersistentData().getCompound("dialogueMetaData");


				List<String> metaDatas = new ArrayList<>(data.getAllKeys());

				for (String timeStamp : metaDatas) {

					int translatedTimestamp = Integer.parseInt(timeStamp);

					if (entity.tickCount - translatedTimestamp > 240) {
						entity.getPersistentData().getCompound("dialogueMetaData").remove(timeStamp);
					}
				}
		...
		if (entity.tickCount % 20 == 0) {
			...
			int damageTier = entity.getPersistentData().getInt("damageDialogueTier");

			// Reset Damagetiers upon regeneration
			if (damageTier > 0) {
				float health = entity.getHealth() / entity.getMaxHealth();

				if (health > 0.3f && damageTier > 2) {
					damageTier = 2;
				}

				if (health > 0.75f && damageTier > 1) {
					damageTier = 1;
				}
				...
				entity.getPersistentData().putInt("damageDialogueTier", damageTier);
			}
