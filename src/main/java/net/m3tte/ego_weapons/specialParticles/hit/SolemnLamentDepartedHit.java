...
public class SolemnLamentDepartedHit extends MetaParticle {
    public SolemnLamentDepartedHit(ClientWorld level, double x, double y, double z, double width, double height, double _null) {
        ...
        this.level.addParticle(EgoWeaponsParticles.SOLEMN_LAMENT_FIRE_DEPARTED.get(), this.x, this.y + 1, this.z, 0.0D, 0.0D, 0.0D);
        double d = 0.2F;
        ...
        for(int i = 0; i < 8; i++) {
            ...
            this.level.addParticle(EgoWeaponsParticles.SOLEMN_LAMENT_DEPARTED_BUTTERFLY.get(), this.x, this.y + 1, this.z, particleMotionX * 0.1, particleMotionY * 0.1, particleMotionZ * 0.1);
        }
    }
