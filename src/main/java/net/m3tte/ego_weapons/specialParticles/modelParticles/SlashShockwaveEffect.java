...
public class SlashShockwaveEffect extends RotationAttackParticle {

    float targetQuadSize = 0;
    Random r = new Random();


    public SlashShockwaveEffect(ClientWorld world, double x, double y, double z, double xSpeed, double targetEntityID, double sourceID, IAnimatedSprite spriteProvider) {
        ...
        this.offset = new Vector3f(0,1f,-0.1f);
        this.targetQuadSize = 2f + r.nextFloat();
        this.quadSize = 0.001f;

        float xOffs = r.nextInt(30) - 15;

        if (this.sourceEntity != null) {

            System.out.println("DEG IS : "+this.sourceEntity.getViewXRot(0));
            xOffs = (xOffs + (this.sourceEntity.getViewXRot(0) * -1)) / 2;

        }

        this.rotation = new Vector3f(xOffs, 90+this.rotation.y() + r.nextInt(50) - 25, r.nextInt(30) - 15);



        this.offsetRate = new Vector3f((float) 0, 0, 0.25f + r.nextFloat() * 0.1f);
    }
    ...
    public static class Provider implements IParticleFactory<BasicParticleType> {
        ...
        public Particle createParticle(BasicParticleType typeIn, ClientWorld worldIn, double x, double y, double z, double xSpeed, double targetID, double sourceID) {
            SlashShockwaveEffect particle = new SlashShockwaveEffect(worldIn, x, y, z, 0, targetID, sourceID, spriteSet);
            return particle;
        }
    }
}
