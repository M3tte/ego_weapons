...
public class TheDepartedButterflyEffect extends CountPotencyStatus {
    ...
    public static void applyOnHit(LivingEntity target, LivingEntity source) {
        ...
        EgoWeaponsMod.PACKET_HANDLER.send(PacketDistributor.ALL.noArg(), new VFXPackages.NumberLabelParticle(target.position().add(target.getRandom().nextFloat() - 0.5f,1,target.getRandom().nextFloat() - 0.5f), NumberParticleTypes.BUTTERFLY, (int) (((potency * departed) / 4f) * mult)));
