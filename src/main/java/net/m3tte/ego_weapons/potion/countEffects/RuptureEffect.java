...
public class RuptureEffect extends CountPotencyStatus {
    ...
    public static void applyOnHit(LivingEntity target) {
        ...
        EgoWeaponsMod.PACKET_HANDLER.send(PacketDistributor.ALL.noArg(), new VFXPackages.NumberLabelParticle(target.position().add(target.getRandom().nextFloat() - 0.5f,1,target.getRandom().nextFloat() - 0.5f), NumberParticleTypes.RUPTURE, potency));
