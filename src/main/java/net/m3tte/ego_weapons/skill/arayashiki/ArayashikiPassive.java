...
public class ArayashikiPassive extends Skill {
    public ArayashikiPassive(Builder<? extends Skill> builder) {
        super(builder);
    }

    private static final UUID EVENT_UUID = UUID.fromString("a355b692-fd97-11eb-9c13-03b2bc141203");
    ...
    public void onInitiate(SkillContainer container) {
        ...
        container.getExecuter().playAnimationSynchronized(FirefistMovesetAnims.FIREFIST_EQUIP, 0);

        if (!guard.isEmpty()) {
            container.getExecuter().getSkillCapability().skillContainers[GenericSkill.TC_GUARD.universalOrdinal()].setSkill(EgoWeaponsSkills.ARAYASHIKI_GUARD);
        }
    }
    ...
    public void onRemoved(SkillContainer container) {
        ...
        if (bsguardskill.hasSkill(EgoWeaponsSkills.ARAYASHIKI_GUARD)) {
