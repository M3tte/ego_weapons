...
public class ABSActiveGuard extends NonSpamGuardSkill {
    public ABSActiveGuard(Builder builder) {
    ...
    public static Builder createBuilder(ResourceLocation resourceLocation) {
        return GuardSkill.createBuilder(resourceLocation)
                .addGuardMotion(EgoWeaponsCategories.ARDOR_BLOSSOM_BLUNT, (item, player) -> ArdorBlossomMovesetAnims.ARDOR_BLOSSOM_GUARD_HIT)
                .addGuardBreakMotion(EgoWeaponsCategories.ARDOR_BLOSSOM_BLUNT, (item, player) -> BlackSilenceMovesetAnims.RANGA_GUARD_STAGGER)
                .addAdvancedGuardMotion(EgoWeaponsCategories.ARDOR_BLOSSOM_BLUNT, (itemCap, playerpatch) ->
                        new StaticAnimation[] { ArdorBlossomMovesetAnims.ARDOR_BLOSSOM_PARRY_1, ArdorBlossomMovesetAnims.ARDOR_BLOSSOM_PARRY_2, ArdorBlossomMovesetAnims.ARDOR_BLOSSOM_PARRY_3, ArdorBlossomMovesetAnims.ARDOR_BLOSSOM_EVADE});
    }
    ...
    public void guard(SkillContainer container, CapabilityItem itemCapability, HurtEvent.Pre event, float knockback, float impact, boolean advanced) {
        if (this.isHoldingWeaponAvailable(event.getPlayerPatch(), itemCapability, BlockType.ADVANCED_GUARD)) {
            ...
            if (this.isBlockableSource(damageSource, true)) {
                ...
                event.getPlayerPatch().playSound(EgoWeaponsSounds.ARDOR_BLOSSOM_PARRY, -0.05F, 0.1F);
