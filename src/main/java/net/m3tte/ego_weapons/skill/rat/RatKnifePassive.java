...
public class RatKnifePassive extends Skill {
    public RatKnifePassive(Builder<? extends Skill> builder) {
        super(builder);
    }
    ...
    public void onInitiate(SkillContainer container) {
        ...
        container.getExecuter().playAnimationSynchronized(RatShankMovesetAnims.RAT_KNIFE_EQUIP, 0);

        if (!guard.isEmpty()) {
            container.getExecuter().getSkillCapability().skillContainers[GenericSkill.TC_GUARD.universalOrdinal()].setSkill(EgoWeaponsSkills.RAT_KNIFE_GUARD);
        }
    }
    ...
    public void onRemoved(SkillContainer container) {
        ...
        if (bsguardskill.hasSkill(EgoWeaponsSkills.RAT_KNIFE_GUARD)) {
