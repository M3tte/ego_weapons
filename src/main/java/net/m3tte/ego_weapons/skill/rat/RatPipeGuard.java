...
import net.m3tte.ego_weapons.EgoWeaponsEFLoader;
...
public class RatPipeGuard extends EnergizingGuardSkill {
    public RatPipeGuard(Builder builder) {
        super(builder);
    }
    ...
    public static Builder createBuilder(ResourceLocation resourceLocation) {
        return GuardSkill.createBuilder(resourceLocation)
                .addAdvancedGuardMotion(EgoWeaponsCategories.RAT_PIPE, (item, player) -> RatPipeMovesetAnims.RAT_PIPE_GUARD_HIT)
                .addGuardMotion(EgoWeaponsCategories.RAT_PIPE, (item, player) -> RatPipeMovesetAnims.RAT_PIPE_GUARD_HIT)
                .addGuardBreakMotion(EgoWeaponsCategories.RAT_PIPE, (item, player) -> BlackSilenceMovesetAnims.RANGA_GUARD_STAGGER);
    }
    ...
    public void guard(SkillContainer container, CapabilityItem itemCapapbility, HurtEvent.Pre event, float knockback, float impact, boolean advanced) {
        ...
        if (this.isBlockableSource(damageSource, advanced)) {
            ...
            if (blockType == BlockType.GUARD_BREAK) {
                if (serverPlayer.getItemInHand(Hand.MAIN_HAND).getOrCreateTag().contains("canKickback")) {
                    event.getPlayerPatch().playSound(EgoWeaponsSounds.METAL_CLASH, 1.0F, 0.0F, 0.1F);
                    animation = RatPipeMovesetAnims.RAT_PIPE_GUARD_BREAK;
                    EgoWeaponsEffects.DEFENSE_LEVEL_UP.get().increment(serverPlayer, 0, 2);
                    EgoWeaponsEffects.TREMOR.get().increment(serverPlayer, 2, 2);
                    serverPlayer.getItemInHand(Hand.MAIN_HAND).getOrCreateTag().remove("canKickback");
                    TremorEffect.burstTremor(serverPlayer, false);
                }
            }
            ...
            EmotionSystem.handleGuard(serverPlayer, event.getAmount(), impact, false, 0.8f-0.02f*tremorLevel, event.getDamageSource().getEntity());
    ...
    public boolean shouldDraw(SkillContainer container) {
        return container.getExecuter().getHoldingItemCapability(Hand.MAIN_HAND).getWeaponCategory() == EgoWeaponsCategories.RAT_PIPE && (Float)container.getDataManager().getDataValue(PENALTY) > 0.0F;
    }
