...
public class FullstopCounterGuard extends EnergizingGuardSkill {
    public FullstopCounterGuard(Builder builder) {
        super(builder);
    }
    ...
    public static Builder createBuilder(ResourceLocation resourceLocation) {
        return GuardSkill.createBuilder(resourceLocation)
                .addAdvancedGuardMotion(EgoWeaponsCategories.FULLSTOP_SNIPER, (item, player) -> FullstopOfficeSniperMovesetAnims.FULLSTOP_SNIPER_PARRY)
                .addGuardMotion(EgoWeaponsCategories.FULLSTOP_SNIPER, (item, player) -> FullstopOfficeSniperMovesetAnims.FULLSTOP_SNIPER_GUARD_HIT)
    ...
    public void guard(SkillContainer container, CapabilityItem itemCapapbility, HurtEvent.Pre event, float knockback, float impact, boolean advanced) {
        ...
        if (this.isBlockableSource(damageSource, advanced)) {
            ...
            if (container.getDataManager().getDataValue(PENALTY) < 0.2 && successParrying) {
                animation = FullstopOfficeSniperMovesetAnims.FULLSTOP_SNIPER_PARRY;
            } else {
            ...
            if (blockType == BlockType.GUARD_BREAK) {
                event.getPlayerPatch().playSound(EgoWeaponsSounds.STAGGER,3,  -0.05F, 0.1F);
    ...
    public boolean shouldDraw(SkillContainer container) {
        return container.getExecuter().getHoldingItemCapability(Hand.MAIN_HAND).getWeaponCategory() == EgoWeaponsCategories.FULLSTOP_SNIPER && (Float)container.getDataManager().getDataValue(PENALTY) > 0.0F;
    }
