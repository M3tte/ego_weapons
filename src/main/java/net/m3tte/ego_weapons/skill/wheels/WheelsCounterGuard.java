...
public class WheelsCounterGuard extends EnergizingGuardSkill {
    public WheelsCounterGuard(Builder builder) {
        super(builder);
    }
    ...
    public static Builder createBuilder(ResourceLocation resourceLocation) {
        return GuardSkill.createBuilder(resourceLocation)
                .addAdvancedGuardMotion(EgoWeaponsCategories.WHEELS_INDUSTRY, (item, player) -> BlackSilenceMovesetAnims.WHEELS_GUARD_HIT)
                .addGuardMotion(EgoWeaponsCategories.WHEELS_INDUSTRY, (item, player) -> BlackSilenceMovesetAnims.WHEELS_GUARD_HIT)
        ...
        if (this.isBlockableSource(damageSource, advanced)) {
            ...
            if (isRudimentaryBlockable(damageSource, advanced) && container.getDataManager().getDataValue(PENALTY) < 0.2f && successParrying) {
                ...
                if (event.getDamageSource().getEntity() instanceof LivingEntity) {
                    ...
                    TremorEffect tremorEffect = TremorEffect.detectTremorType(entitySource);
                    int potency = 0;
                    if (tremorEffect != null) {
                        potency = tremorEffect.getPotency(entitySource);
                    }

                    if (potency > 5) {
                        event.getPlayerPatch().playSound(EgoWeaponsSounds.GUARD_WIN, -0.05F, 0.1F);
                        ...
                        tremorEffect.burst(entitySource);
                        tremorEffect.decrement(entitySource, 1, 0);
                        if (entitypatch != null && !StaggerSystem.isStaggered(entitySource)) {
            ...
            if (container.getDataManager().getDataValue(PENALTY) < 0.2 && successParrying) {
                animation = BlackSilenceMovesetAnims.WHEELS_GUARD_COUNTER;
            } else {
    ...
    public boolean shouldDraw(SkillContainer container) {
        return container.getExecuter().getHoldingItemCapability(Hand.MAIN_HAND).getWeaponCategory() == EgoWeaponsCategories.WHEELS_INDUSTRY && (Float)container.getDataManager().getDataValue(PENALTY) > 0.0F;
    }
