...
public class LCAReloadAbility extends ReloadAbility {
    ...
    public void trigger(PlayerEntity player, PlayerVariables playerVars, ItemStack ammoItem) {
        ...
        if (canTrigger(player, playerVars)) {
            ...
            if (player.getItemInHand(Hand.MAIN_HAND).getItem().equals(EgoWeaponsItems.LCA_RIFLE.get())) {
                entitypatch.playAnimationSynchronized(LCARifleMovesetAnims.LCA_UDJAT_RELOAD_MH, 0.1f);
            } else {
                entitypatch.playAnimationSynchronized(LCARifleMovesetAnims.LCA_UDJAT_RELOAD_OH, 0.1f);
            }
