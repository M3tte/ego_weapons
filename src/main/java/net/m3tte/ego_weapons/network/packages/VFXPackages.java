...
import static net.m3tte.ego_weapons.client.renderer.EgoWeaponsRenderSystem.incrementShockwaveTarget;

public class VFXPackages {
            ...
            context.enqueueWork(() -> {
                if (!context.getDirection().getReceptionSide().isServer()) {
                    ...
                    if (target != null) {
                        ...
                        EgoWeaponsMod.LOGGER.warn("Target for insanity is null.");
                    }
            ...
            context.enqueueWork(() -> {
                if (!context.getDirection().getReceptionSide().isServer()) {
                    ...
                    if (target != null && source != null) {
                        Minecraft.getInstance().level.addParticle((IParticleData) Objects.requireNonNull(ForgeRegistries.PARTICLE_TYPES.getValue(message.particleID)), target.getX(), target.getY(), target.getZ(), 0, source.getId(), 0);

                    } else {
                        EgoWeaponsMod.LOGGER.warn("Target for directional particle is null.");
                    }
            ...
            context.enqueueWork(() -> {
                if (!context.getDirection().getReceptionSide().isServer()) {
                    ...
                    if (target != null) {
                        ...
                        EgoWeaponsMod.LOGGER.warn("Target for magic bullet particle is null.");
                    }
    ...
    public static class ShockwaveShakePackage {

        float shockwaveAmount = 0;
        float shakeStrength = 0;
        float shakeSpeed = 0;
        float fov = 0;
        public ShockwaveShakePackage(float shockwaveAmount, float shakeStrength, float shakeSpeed, float fov) {
            this.shockwaveAmount = shockwaveAmount;
        ...
        public ShockwaveShakePackage(PacketBuffer buffer) {
            this.shockwaveAmount = buffer.readFloat();
            this.shakeStrength = buffer.readFloat();
            this.shakeSpeed = buffer.readFloat();
            this.fov = buffer.readFloat();

        }

        public static void buffer(ShockwaveShakePackage message, PacketBuffer buffer) {
            buffer.writeFloat(message.shockwaveAmount);
            buffer.writeFloat(message.shakeStrength);
            buffer.writeFloat(message.shakeSpeed);
            buffer.writeFloat(message.fov);
        }

        public static void handler(ShockwaveShakePackage message, Supplier<NetworkEvent.Context> contextSupplier) {
            NetworkEvent.Context context = contextSupplier.get();
            context.enqueueWork(() -> {
                if (!context.getDirection().getReceptionSide().isServer()) {
                    if (message.shockwaveAmount > 0)
                        incrementShockwaveTarget(message.shockwaveAmount);
                    if (message.shakeSpeed > 0 || message.shakeStrength > 0) {
                        EgoWeaponsRenderSystem.applyScreenshake(message.shakeSpeed, message.shakeStrength);
                    }
                    if (message.fov > 0)
                        EgoWeaponsRenderSystem.applyFOV(message.fov);
                }
            });
            context.setPacketHandled(true);
        }
