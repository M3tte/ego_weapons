...
public class EgoWeaponsModVars {
	public EgoWeaponsModVars() {
	...
	public void onPlayerLoggedIn(PlayerEvent.PlayerLoggedInEvent event) {
		if (!event.getPlayer().level.isClientSide) {
			...
			if (mapdata != null)
				EgoWeaponsMod.PACKET_HANDLER.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) event.getPlayer()),
	...
	public static class WorldVariables extends WorldSavedData {
		public static final String DATA_NAME = "ego_weapons_worldvars";
		...
		public void syncData(IWorld world) {
			...
			if (world instanceof World && !((World) world).isClientSide)
				EgoWeaponsMod.PACKET_HANDLER.send(PacketDistributor.DIMENSION.with(((World) world)::dimension),
	...
	public static class MapVariables extends WorldSavedData {
		public static final String DATA_NAME = "ego_weapons_mapvars";
		...
		public void syncData(IWorld world) {
			this.setDirty();
			if (world instanceof World && !((World) world).isClientSide)
				EgoWeaponsMod.PACKET_HANDLER.send(PacketDistributor.ALL.noArg(), new WorldSavedDataSyncMessage(0, this));
		}
	...
	public void onAttachCapabilities(AttachCapabilitiesEvent<Entity> event) {
		if (event.getObject() instanceof PlayerEntity && !(event.getObject() instanceof FakePlayer))
			event.addCapability(new ResourceLocation( EgoWeaponsMod.MODID, "player_variables"), new PlayerVariablesProvider());
	}
	...
	public static class PlayerVariables {
		...
		public void syncPlayerVariables(Entity entity) {
			if (entity instanceof ServerPlayerEntity)
				EgoWeaponsMod.PACKET_HANDLER.send(PacketDistributor.PLAYER.with(() -> (ServerPlayerEntity) entity), new PlayerVariablesSyncMessage(this));
		}
		...
		public void syncEmotionLevel(Entity entity) {
			if (entity instanceof ServerPlayerEntity) {
				for (ServerPlayerEntity p : ServerLifecycleHooks.getCurrentServer().getPlayerList().getPlayers()) {
					EgoWeaponsMod.PACKET_HANDLER.send(PacketDistributor.PLAYER.with(() -> p), new SyncEmotionLevelMSG(this, (ServerPlayerEntity)entity));
				}
			}
		}
		...
		public void syncSanity(Entity entity) {
			if (entity instanceof ServerPlayerEntity) {
				for (ServerPlayerEntity p : ServerLifecycleHooks.getCurrentServer().getPlayerList().getPlayers()) {
					EgoWeaponsMod.PACKET_HANDLER.send(PacketDistributor.PLAYER.with(() -> p), new SyncSanityMessage(this, (ServerPlayerEntity)entity));
				}
			}
		}
	}
	...
	public static class SyncCountEffectMessage {
		...
		public static void buffer(SyncCountEffectMessage message, PacketBuffer buffer) {
			...
			buffer.writeInt(message.effectDuration);
			buffer.writeInt(message.effectPotency);
			buffer.writeInt(message.targetID);
		}

		public static void handler(SyncCountEffectMessage message, Supplier<NetworkEvent.Context> contextSupplier) {
			NetworkEvent.Context context = contextSupplier.get();
			context.enqueueWork(() -> {
				if (!context.getDirection().getReceptionSide().isServer()) {
					if (Minecraft.getInstance().level == null)
					...
					Entity entity = Minecraft.getInstance().level.getEntity(message.targetID);
