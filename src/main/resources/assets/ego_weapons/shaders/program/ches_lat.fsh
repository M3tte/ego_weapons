...
void main() {
    ...
    float distortion = sin(uv.y * 12.0 + GameTime * 2.5) * 0.03;
    ...
    // vignette mask
    float vignette = smoothstep(0.0, 0.85, dist);
    ...
    // red glow
    vec3 chesedTint = vec3(0.2 + (n3 / 2), 0.2 + (n3 / 2), 0.8 + (n3 / 2));


    // apply effect only near edges
    color.rgb += chesedTint * vignette * (1 + noiseFX);
